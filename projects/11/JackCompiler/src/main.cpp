...
int main(int argc, char* argv[]) 
{
  ...
  if (fs::is_directory(inPath)) 
  {
    ...
    // 4) Set the output directory to "out" inside the input directory
    outputDir = inPath / "out";
  ...
  else if (fs::is_regular_file(inPath)) 
  {
    ...
    // 4) Set output directory to "out" at the same level as the input file
    outputDir = inPath.parent_path() / "out";
    if (!fs::exists(outputDir)) 
    {
      fs::create_directory(outputDir);
    }
  ...
  // 5) Process the files using JackCompiler

  JackCompiler compiler(inputFiles, outputDir);
  ...
  std::cout << "Compilation complete. Output files have been created in directory: " 
            << outputDir << std::endl;
